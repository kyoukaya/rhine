...
// send notification events via callbacks while walking a json input.
func (mod *GameState) WalkAndNotify(data []byte) error {
	...
	for i, c := range data {
		...
		depth := len(scan.parseState)
		if step == scanBeginLiteral {
			lastLiteralPos = i
		} else if step == scanObjectKey {
